...
**§3.11-Metrik UMGESTELLT (erledigt 2026-06-28, CLAUDE.md v4.31):** Budget-Ziel jetzt **~60% Session-Limit (UI-%)** statt ~150k Token (Token vom Modell nicht messbar; empirisch 12 Posten ≈ 58%). **Offen/groß:** natives Xing/Jobware-Scraping (Wolf, eigene Session). **§3.6a (alt):** echter externer ATS-Parser-Durchlauf.
**Ziel-Session:** Claude Code CLI (optional Scraping = eigene Session/Wolf; sonst Pipeline produktiv nutzen: `ANZEIGE_DATEI` setzen + Generator laufen)
