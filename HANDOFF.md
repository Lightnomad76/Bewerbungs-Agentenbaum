...
- **Agenten-Roadmap E-C Teil 2/3 — getrennte Writer + ATS (Marek/Wolf): NÄCHSTE Etappe(n)**, eigene frische Session. Teil 2: CoverLetterWriter + CVTailoring (konsumieren JDParser-`parse()`). Teil 3: ATS-Zwei-Pfad fest in ReportAgent (ggf. Wolf). Grundlage `state/agenten_roadmap.md` A.4 + C.
