...
**Phase:** **Etappe v9 (E-C Teil 3 = ATS-Linter, Marek) ABGESCHLOSSEN+COMMITTED** — CV-`.docx` ATS-Risiko-Prüfung (Tabellen/Mehrspaltig/Textbox/Header-Kontakt/Scan-CV); 60k-Schnitt (User 2026-06-28). **Nächstes: Mini-Fix abschluss-Meta, dann Generator-Integration + Text-Writer (eigene Sessions).**
**Ziel-Session:** Claude Code CLI (nächste Etappe = Generator-Integration ODER Text-Writer / Marek)
...
## Sofort-Einstieg NÄCHSTE Etappe — Generator-Integration ODER Text-Writer (frischer Chat, Marek)

Etappe 1–9 fertig (Engine + MatchAgent + Web-UI + CriticAgent + FactGroundingAgent + crewai-Fix +
**JDParserAgent** + **TailoringAgent/GapAnalysis** + **ATS-Linter**). Die deterministischen Agenten
stehen; offen ist Verdrahtung + Text-Generierung. Reihenfolge:
0. **ZUERST lesen:** `state/etappe_v9_state.md` + `state/etappe_v8_state.md` + `state/etappe_v7_state.md`
   (die drei E-C-Agenten) + `state/agenten_roadmap.md` (A.4 + B + C).
1. **Environment-Smoke (§3.11):**
   - venv-Python: `verify_engine.py` + `verify_match.py` + `verify_critic.py` + `verify_factground.py`
     + `verify_jdparser.py` + `verify_tailoring.py` exit 0.
   - global `py -3.11`: `py -3.11 verify_ats_lint.py` exit 0 (+ `py -3.11 -c "import docx"`).
2. **Mit Marek eine** der zwei Stoßrichtungen schneiden:
   - **(a) Generator-Integration** — jdparser/tailoring/ats_lint in `profile/gen_bewerbung_guetepruefer.py`
     (lokal/PII/gitignored) einhängen wie critic/factground; Kostentreiber = Generator lesen.
   - **(b) Text-Writer** (eigene ~150k-Session) — CoverLetterWriter + CVTailoring-Text, konsumieren
     `parse()` + `abgleich()`; Stil-Regeln aus agenten_roadmap.md Abschnitt B.
3. Selbstcheck (§3.5) → auf „go"/„ZIP" warten → `state/etappe_v<N>_state.md`.
- **Kleiner offener Mini-Fix:** jdparser `abschluss`-Meta (nachlaufendes Trigger-Wort) — ggf. mitnehmen.
- **Optional/Wolf:** natives Xing/Jobware-Scraping (nur falls Google-Umweg-Abdeckung dünn).
- **Muster (belegt):** critic.py + factground.py + jdparser.py + tailoring.py + **ats_lint.py** sind die
  Vorbilder (deterministisch, offline, importierbare API + CLI + eigenes `verify_*.py`, flaggt-ändert-nichts).
...
- **Generator-Integration (Marek): NÄCHSTE Etappe (eigene Session).** jdparser + tailoring + ats_lint in `profile/gen_bewerbung_guetepruefer.py` einhängen (lokal/PII). Kostentreiber = Generator lesen.
